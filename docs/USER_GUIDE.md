...
Output: **`src-tauri/target/release/bundle/msi/`** — look for `Nexttorrent_1.1.0_x64_en-US.msi` (exact filename follows version and locale from `tauri.conf.json`). An NSIS setup `.exe` may appear under `bundle/nsis/` when `bundle.targets` includes it (`all` does).
